...
for i = 0, num_sel_tracks - 1 do
  ...
  for j = 0, reaper.CountTracks(0) - 1 do
    ...
    -- Check if the track name starts with "FX4"
    track_name_ok, track_name = reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "", false)
    if track_name_ok and string.sub(track_name, 1, 3) == "FX4" then

      -- Check if the selected track already has a send to the FX4 track
      has_send = false
      ...
      -- If the selected track doesn't already have a send to the FX4 track, create one
      ...
      break -- Exit the loop after the first "FX4" track is found
