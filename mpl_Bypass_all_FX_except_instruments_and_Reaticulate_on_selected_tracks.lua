...
--    # reverted native first(only) instrument check
  ...
  function IsReaticulate(track, fx)  return ({reaper.TrackFX_GetFXName( track, fx, '' )})[2]:match('Reaticulate')~= nil end
  ...
  function SetFXState(FX_state, t)
    for i = 1, CountSelectedTracks(0) do
      ...
      for fx_id = 1, TrackFX_GetCount(tr) do
        if not IsInstrument(tr, fx_id-1) and not IsReaticulate(tr, fx_id-1) then
          -- check t
